...
class _SavedResourcesScreenState extends State<SavedResourcesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(l10n.savedResourcesTitle),
        leading: IconButton(
      ...
      body: BlocBuilder<ProfileCubit, ProfileState>(
        builder: (context, state) {
          if (state is SavedResourcesLoading) return const Center(child: CircularProgressIndicator());
          ...
          if (state is SavedResourcesSuccess) {
            if (state.resources.isEmpty) {
              return Center(child: Text(l10n.noSavedResources, style: const TextStyle(color: Colors.grey)));
            }
