...
class DoctorPatientsApiService {
  ...
  Future<List<DoctorPatientModel>> searchDoctorPatients(String query) async {
    return _fetchPatients('/api/DoctorPatients/search', queryParameters: {'query': query});
  }

  Future<List<DoctorPatientModel>> _fetchPatients(String endpoint, {Map<String, dynamic>? queryParameters}) async {
    try {
      ...
      if (data is List) {
        return data.map((json) => DoctorPatientModel.fromJson(Map<String, dynamic>.from(json))).toList();
      } 
      ...
      if (data is Map) {
        ...
        if (items is List) {
          return items.map((json) => DoctorPatientModel.fromJson(Map<String, dynamic>.from(json))).toList();
        }
      }

      return [];
    } on DioException catch (e) {
      log('DioError at $endpoint: ${e.response?.statusCode} - ${e.response?.data}');
      rethrow;
    } catch (e) {
