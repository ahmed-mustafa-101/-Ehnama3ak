...
class _TherapistsPageState extends State<TherapistsPage> {
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        ...
        return StatefulBuilder(builder: (context, setState) {
          ...
          return Padding(
            padding: EdgeInsets.only(left: 24, right: 24, top: 24, bottom: 24 + bottomInsets),
            child: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('${l10n.bookSessionWith} ${doctor.name}',
                      style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16),
                  Text(l10n.sessionType, style: const TextStyle(fontWeight: FontWeight.bold)),
                  DropdownButton<String>(
                    isExpanded: true,
                    value: selectedType,
                    items: ['Video', 'Audio', 'Chat'].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
                    onChanged: (val) { if (val != null) setState(() => selectedType = val); },
                  ),
                  const SizedBox(height: 16),
                  Text(l10n.sessionDate, style: const TextStyle(fontWeight: FontWeight.bold)),
                  ListTile(
                    contentPadding: EdgeInsets.zero,
                    title: Text(selectedDate == null ? l10n.selectDate : DateFormat('yyyy-MM-dd').format(selectedDate!)),
                  ...
                  Text(l10n.sessionTime, style: const TextStyle(fontWeight: FontWeight.bold)),
                  ListTile(
                    contentPadding: EdgeInsets.zero,
                    title: Text(selectedTime == null ? l10n.selectTime : selectedTime!.format(context)),
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        if (selectedDate == null || selectedTime == null) {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.pleaseSelectDateTime)));
                          return;
                        }
                        final dt = DateTime(selectedDate!.year, selectedDate!.month, selectedDate!.day,
                            selectedTime!.hour, selectedTime!.minute);
                        Navigator.pop(context);
                        context.read<DoctorCubit>().bookSession(doctor.id, dt.toUtc().toIso8601String(), selectedType);
                      },
    ...
    return BlocListener<DoctorCubit, DoctorState>(
      listener: (context, state) {
        if (state is BookSessionSuccess) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.bookingSession), duration: const Duration(milliseconds: 500)));
        }
      },
      child: Scaffold(
        body: SafeArea(
            ...
            children: [
              ...
              Expanded(
                child: BlocBuilder<DoctorCubit, DoctorState>(
                  buildWhen: (p, c) => c is DoctorLoading || c is DoctorSuccess || c is DoctorInitial || c is DoctorError,
                  builder: (context, state) {
                    if (state is DoctorLoading || state is DoctorInitial) return const Center(child: CircularProgressIndicator());
                    if (state is DoctorError) return _buildErrorState(state.message, l10n);
          ...
          children: [
            Container(
              ...
              child: TextField(
                ...
                onChanged: (val) => context.read<DoctorCubit>().searchDoctors(val),
                decoration: InputDecoration(
                  ...
                  suffixIcon: _searchController.text.isNotEmpty
                      ? IconButton(icon: const Icon(Icons.clear, color: Colors.white70),
                          onPressed: () { _searchController.clear(); context.read<DoctorCubit>().loadDoctors(); })
                      : null,
                ),
              ),
            ),
            const SizedBox(height: 12),
            Expanded(child: Center(child: Image.asset('assets/images/therapy.png', fit: BoxFit.contain,
  ...
  Widget _buildErrorState(String message, AppLocalizations l10n) {
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const SizedBox(height: 16),
      ElevatedButton(onPressed: () => context.read<DoctorCubit>().loadDoctors(), child: Text(l10n.retry)),
    ]));
  }
  ...
  Widget _buildEmptyState(AppLocalizations l10n) {
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const SizedBox(height: 16),
      Text(l10n.noDoctorsFound, style: const TextStyle(color: Colors.grey, fontSize: 16)),
    ]));
  }
                ...
                children: [
                  Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    CircleAvatar(radius: 36,
                        ...
                        child: doctor.imageUrl.isEmpty ? const Icon(Icons.person, size: 36) : null),
                    const SizedBox(width: 16),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(doctor.name, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 4),
                      Text(doctor.specialization, style: const TextStyle(fontSize: 14, color: Colors.blueAccent)),
                      ...
                      Row(children: [
                        ...
                        const SizedBox(width: 16),
                        const Icon(Icons.work_outline, color: Colors.grey, size: 18),
                        const SizedBox(width: 4),
                        Text('${doctor.experienceYears} ${l10n.years}', style: const TextStyle(color: Colors.grey)),
                      ]),
                    ])),
                  ]),
                  const SizedBox(height: 16),
