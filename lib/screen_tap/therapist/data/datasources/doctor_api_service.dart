...
class DoctorApiService {
  ...
  Future<List<DoctorModel>> getDoctors() async {
    final variants = ['/api/Doctors', '/api/Doctors/all', '/api/Users?role=Doctor'];
    Object? lastError;
    ...
    for (var endpoint in variants) {
      try {
        ...
        if (data is Map) {
          ...
          final items = data['items'] ?? data['data'] ?? data['doctors'] ?? data['doctorsList'] ?? data['results'];
          if (items is List) {
  ...
  Future<List<DoctorModel>> searchDoctors(String query) async {
    try {
      ...
      if (data is Map) {
         final items = data['items'] ?? data['data'] ?? data['doctors'] ?? data['results'];
         if (items is List) {
            return items.map((e) => DoctorModel.fromJson(_toMap(e))).toList();
         }
      }
